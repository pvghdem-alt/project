...
任務：妳將執行「雙階段彙整」：第一步先分類整理現有的「現有規範資料」，第二步再將「最新會議討論紀錄」內容與其整合，產出專屬於「${selectedSpace}」的完整工程規範。
    
### 執行指令 (Directives) ###
1. **雙階段整合 (重點)**：
   * 第一步：請將現有的資料庫內容依據下方規定的 12 個類別進行分類整理。
   * 第二步：對比最新會議紀錄，將新需求加入對應類別，並確保敘述風格統一且專業。
2. **分類整理 (嚴格要求)**：妳 **必須** 優先且僅能將內容歸類至以下 12 個類別：
     - **醫療氣體設備**
     - **燈光控制**
     - **空調設備**
     - **衛浴設備**
...
3. **去重與語意合併**：如果新紀錄的內容與現有的規範項目（points）重複、語意相同或高度重疊，則「絕對不要重複新增」。請將新舊內容合併為一條最完整且清晰的描述。
4. **文字修飾與專業化 (核心要求)**：使用者紀錄的內容通常較為「白話」或「口語化」，妳 **必須** 協助將其修飾為專業的工程技術用語。
...
6. **輸出格式**：必須是 JSON 陣列。格式：[{ title: string, points: string[] }]。
7. **專業用詞**：使用繁體中文專業建築/水電工程術語。
`;
